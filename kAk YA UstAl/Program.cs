class Program
{
    static void Main()
    {
        ...
        while (true)
        {
            ...
            var key = ArrowKeysHandler.WaitForArrowKeys();
            if (key == ConsoleKey.UpArrow && selectedDriveIndex > 0)
            {
                selectedDriveIndex--;
            }
            else if (key == ConsoleKey.DownArrow && selectedDriveIndex < drives.Length - 1)
            {
                selectedDriveIndex++;
            }
    ...
    static void DisplayDriveContents(DriveInfo drive)
    {
        ...
        while (true)
        {
            ...
            var key = ArrowKeysHandler.WaitForArrowKeys();
            if (key == ConsoleKey.UpArrow && selectedEntryIndex > 0)
            {
                selectedEntryIndex--;
            }
            else if (key == ConsoleKey.DownArrow && selectedEntryIndex < entries.Length - 1)
            {
                selectedEntryIndex++;
            }
            else if (key == ConsoleKey.Enter)
